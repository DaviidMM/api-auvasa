  ...
  const rtStopTimes = (await getCachedRtStopTimes())
    ...
    .map(async rtSt => {
      ...
      if (rtSt.arrival_timestamp) {
        ...
        // TODO: Si no hay datos RT deberíamos buscar si en las stop_sequence anteriores
        // hay datos RT, y anotar el desfase para aplicarlo a la actual
        // https://github.com/BlinkTagInc/node-gtfs/issues/162
        // TODO: usar getTripSequence() y filtrar por el stop_sequence
        return [];
      }
...
const getPropagatedDelay = async (tripId, stopSequence) => {
  try {
    ...
    // Filtrar las stoptimes con datos en tiempo real
    const realTimeStops = stoptimes.filter((stoptime) => {
      const realTimeArrival = gtfs.getStopTimeUpdates({
        stop_id: stoptime.stop_id,
        trip_id: tripId,
      });
      return realTimeArrival.length > 0 && realTimeArrival[0].arrival_timestamp;
    });

    // Encontrar la última parada con datos en tiempo real antes de stopSequence
    const lastRealTimeStop = realTimeStops
      .filter((stoptime) => stoptime.stop_sequence < stopSequence)
      .sort((a, b) => b.stop_sequence - a.stop_sequence)[0];

    if (!lastRealTimeStop) {
      // Si no hay paradas anteriores con datos en tiempo real, devolver null
      return null;
    }

    const scheduledArrival = getFullArrivalDateTime(
      lastRealTimeStop.arrival_time,
    );
    const realTimeArrival = new Date(
      lastRealTimeStop.arrival_timestamp * 1000,
    ).toISOString();
    const updatedArrival = moment
      .tz(realTimeArrival, 'UTC')
      .tz('Europe/Madrid')
      .format('YYYY-MM-DDTHH:mm:ssZ');
    ...
    const delayMinutes =
      Math.sign(delaySeconds) * Math.abs(Math.floor(delaySeconds / 60));
      ...
      .map(async (stoptime) => {
        ...
        if (
          ...
          if (scheduled_arrival && propagatedDelay) {
            updated_arrival = moment(scheduled_arrival)
        ...
        if (scheduled_arrival && updated_arrival) {
          const shiftSeconds =
            (new Date(updated_arrival) - new Date(scheduled_arrival)) / 1000;
          const shiftMinutes =
            Math.sign(shiftSeconds) * Math.abs(Math.floor(shiftSeconds / 60));
