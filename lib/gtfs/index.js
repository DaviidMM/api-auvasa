...
const gtfsGetTripSequence = async (tripId) => {
  try {
    // Query for the stop times for the given trip_id
    const stoptimes = gtfs.getStoptimes({ trip_id: tripId });

    // If no stoptimes found, return an empty array
    if (stoptimes.length === 0) {
      return [];
    }

    // Extract stop_ids from stoptimes
    const stopIds = stoptimes.map(stoptime => stoptime.stop_id);

    // Query for the stops using the stop_ids
    const stops = gtfs.getStops({ stop_id: stopIds });

    // Map stoptimes to their corresponding stops and sort by stop_sequence
    const stopsWithSequence = stoptimes
      .map((stoptime) => {
        ...
        if (scheduledArrival.length > 0) {
          scheduled_arrival = getFullArrivalDateTime(scheduledArrival[0].arrival_time);
        }
        ...
        if (
          ...
          // Si no hay datos en tiempo real buscamos la primera parada anterior que los tenga
          // y calculamos el desfase con el que viene, se lo aplicamos la hora programada de esta parada
          // getPropagatedDelay(tripId, stoptime.stop_sequence)
        }

        let delay;
        if (scheduled_arrival && updated_arrival) {
          const shiftSeconds = (scheduled_arrival - updated_arrival) / 1000;
          // Calcular el desfase en minutos, manteniendo el signo
          const shiftMinutes =
            Math.sign(shiftSeconds) * Math.abs(Math.floor(shiftSeconds / 60));
