...
const gtfsGetBusOccupancy = async (tripId) => {
  const key = `busOccupancy:${tripId}`;
  try {
    // Obtener la URL del feed GTFS Realtime para el ocupamiento de vehículos
    const realtimeUrl = gtfsConfig.agencies[0].realtimeUrls[1];
    // Obtener el feed GTFS Realtime
    const response = await fetch(realtimeUrl);
    const buffer = await response.arrayBuffer();

    try {
      // Intentar decodificar el feed GTFS Realtime
      const feed = gtfsRealtimeBindings.transit_realtime.FeedMessage.decode(
        new Uint8Array(buffer),
      );
      // Buscar el ocupamiento del vehículo para el tripId dado
      const vehicleOccupancy = feed.entity.find(
        (entity) =>
          entity.vehicle &&
          entity.vehicle.trip &&
      ...
      const occupancyStatus = vehicleOccupancy.vehicle.occupancyStatus;
      // Para evitar problemas comprobamos que el estado recibido es un número
      // entre 0 y 7, y no datos corruptos
      if (
        typeof occupancyStatus === 'number' &&
        occupancyStatus >= 0 &&
        occupancyStatus <= 7
      ) {
        const cachedOccupancyStatus = occupancyStatusMapping[occupancyStatus];
        // Almacenar el estado válido actual en el caché
        myCache.set(key, cachedOccupancyStatus.toString());
        return cachedOccupancyStatus;
      } else {
        const cachedValue = myCache.get(key);
        if (cachedValue && typeof cachedValue == 'string') {
          return cachedValue;
        } else {
          return null; // Si no hay estado anterior en el caché, devolver null
        }
      }
    } catch (error) {
      // En caso de error al decodificar, que pasa habitualmente
      // buscar en el caché el estado anterior
      const cachedValue = myCache.get(key);
      if (cachedValue && typeof cachedValue == 'string') {
        return cachedValue;
      } else {
    ...
    console.error(
      `Error al obtener el estado de ocupación para el tripId ${tripId}:`,
      error,
    );
    // En caso de otro error, buscar en el caché el estado anterior
    const cachedValue = myCache.get(key);
    if (cachedValue && typeof cachedValue == 'string') {
      return cachedValue;
    } else {
