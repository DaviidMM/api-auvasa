...
const gtfsGetStop = async (stopNumber, routeShortName) => {
  ...
  const trips = await gtfs.getTrips({
    trip_id: stopTimes.map((stopTime) => stopTime.trip_id),
  });
  const gtfsRoutes = await gtfs.getRoutes({
    route_id: trips.map((trip) => trip.route_id),
  });

  // Filtrar las rutas por el número de línea
  const routes = routeShortName
    ? gtfsRoutes.filter((route) => route.route_short_name === routeShortName)
    : gtfsRoutes;

  // Obtenemos los horarios de cada línea
  // Si hay actualizaciones en tiempo real, se añaden al objeto
  // Si no hay actualizaciones en tiempo real, se devuelve el primer horario
...
const getRTStopTimes = async ({ stopTimes, route }) => {
  const rtStopTimes = (await getCachedRtStopTimes())
    // Filtrar las actualizaciones en tiempo real que coinciden con los horarios
    .filter((rtSt) =>
      stopTimes.find(
        (sT) =>
          sT.trip_id === rtSt.trip_id &&
          sT.stop_sequence === rtSt.stop_sequence,
      ),
    )
    ...
    .map((rtSt) => {
      // sT.arrival_timestamp es una string en formato UTC
      // Covertimos la hora a la zona horaria Europe/Madrid
      const arrivalTime = moment
        .tz(rtSt.arrival_timestamp, 'UTC')
        .tz('Europe/Madrid')
        .format('HH:mm:ss');
      const scheduledStop = stopTimes.find(
        (sT) =>
          sT.trip_id === rtSt.trip_id &&
          sT.stop_sequence === rtSt.stop_sequence,
      );
