...
const { GTFS_REFRESH_RATE, GTFS_STATIC_REFRESH_RATE, GTFS_RT_CACHE } = environment;
const gtfsConfig = require('./config');
const moment = require('moment-timezone');
const { importGtfs } = require('./helpers/import');
...
const initializeGtfs = async () => {
  return new Promise((resolve) => {
    import('gtfs').then(async (module) => {
      ...
      // Descargar e importar estáticos
      async function updateStaticGtfs() {
        try {
          await importGtfs();
        } catch (error) {
          console.error('Error al importar estáticos importGtfs():', error);
        }

        // Descargar e importar de nuevo cada GTFS_STATIC_REFRESH_RATE ms
        setTimeout(updateStaticGtfs, GTFS_STATIC_REFRESH_RATE || 43200000); //12h por defecto
      }

      // Ejecutar inmediatamente la primera vez
      await updateStaticGtfs();
      ...
      // Actualizar los datos cada GTFS_RT_REFRESH_RATE milisegundos
      setInterval(async () => {
        await updateGtfsRt();
      }, GTFS_REFRESH_RATE || 60000); // 60s si no hay config
